...
struct Options {
  uint64_t seed = 42;  topoi::index_t levels = 6;
  topoi::index_t sample_moves = 10;
  template <class App>
  void setup_onto(App& app) {
    // clang-format off
      app.add_option("--seed", seed, "Random seed");      app.add_option("--levels", levels, "Levels");
      app.add_option("--sample-moves", sample_moves, "Sample moves");
    // clang-format on
  }
};
